...
public class UserTest {
    ...
    public void trySaveAllModelsMappedToEachOther() {
        ...
        //map Employee with User and vice versa
        ...
        //map cartAccessAttr to entityAttr and vice versa
        cartAccessAttr.setEntityAttr(entityAttr1);
        cartAccessAttr.setEntityAttr(entityAttr2);
        entityAttr1.setCartAccessAttr(cartAccessAttr);
        entityAttr2.setCartAccessAttr(cartAccessAttr);
        ...
        cartAccessAttr.setRightsUseRule(rightsUseRule);

        //save User
        User savedUser1 = userRepository.save(user1);
        User savedUser2 = userRepository.save(user2);

        //check to get user from DB
        User userFromDB = userRepository.findById(savedUser1.getId()).orElse(null);
        //check that user exist
        assertNotNull(userFromDB);
        //check that user have employee and employee have name and etc
        assertEquals(
        ...
        CartAccess cartAccessFromDB = groupAccessFromDB.getCartAccesses().get(0);
        assertNotNull(cartAccessFromDB.getEntity());
        Entity entityFromDB = cartAccessFromDB.getEntity();
        //check that Entity have RuleSelectCart with same sql string
        assertNotNull(entityFromDB.getRuleSelectCart());
        assertEquals(
                entityFromDB.getRuleSelectCart().getSqlString(),
                ruleSelectCart.getSqlString());
        //check that entity have two attributes
        assertNotNull(entityFromDB.getEntityAttrList());
        assertEquals(entityFromDB.getEntityAttrList().size(), 2);
        EntityAttr entityAttrFromDB = entityFromDB.getEntityAttrList().get(0);
        //check that entityAttr have cartAccessAttr
